...
In the Entry field at the top of the screen (it says *Expression (press Shift+Enter for newlines)*) type the following query:
...
Now delete the previous query and type:
...
To see the total requested storage you can use the *sum* operator. So type the following:
...
And if you want to see how many persistent volume claims are *Bound* or *Pending* you can use the following two queries:
...
Back in your SSH Shell switch to the `root` user:
...
[source,bash,role="copypaste"]
----
tb rados -p replicapool ls | grep pvc
----

No Output returned because there aren't any objects in the replicapool yet.
...
glusterfs-storage (default)   kubernetes.io/glusterfs   2h
rook-ceph-block               ceph.rook.io/block        5m
----
...
Replace `xxxxxxxxxxx` with your unique value and copy the URL to your browser to create articles.
...
Lets now take another look at the replicapool created by the OCP storageclass. Use the toolbox pod again. Run the same Ceph commands as before the application deployment and compare to results in prior section.
...
GLOBAL:
    SIZE       AVAIL      RAW USED     %RAW USED
    150GiB     147GiB      3.11GiB          2.07
POOLS:
    NAME            ID     USED        %USED     MAX AVAIL     OBJECTS
    replicapool     1      68.3MiB      0.10       69.7GiB          33
----
...
----
POOL_NAME     USED OBJECTS CLONES COPIES MISSING_ON_PRIMARY UNFOUND DEGRADED RD_OPS      RD WR_OPS     WR
replicapool 68 MiB      33      0     66                  0       0        0    224 987 KiB   1219 60 MiB
...
Notice the number of objects in replicapool now.
...
[source,texinfo]
----
rbd_id.pvc-2d017d1d-6ea3-11e9-8dce-02cae411299c
----
...
----
NAME         STATUS    VOLUME                                     CAPACITY   ACCESS MODES   STORAGECLASS      AGE
postgresql   Bound     pvc-2d017d1d-6ea3-11e9-8dce-02cae411299c   5Gi        RWO            rook-ceph-block   4m
----
