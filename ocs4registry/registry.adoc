...
Lets review the current Registry Settuing first. To do so, please run the command:
...
NOTE: The `storage` designation currently refers to `s3` and a `bucket` name. This is   initial deployment of OCP4 on AWS infrastructure.
...
NOTE: This method of moving the registry to OCS will work exactly the same for OCP on VMware infrastructure.
