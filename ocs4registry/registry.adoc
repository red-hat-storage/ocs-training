...
Alternate method to use the OCS storage can be done using the `oc patch` command.

[source,role="execute"]
----
oc patch configs.imageregistry.operator.openshift.io cluster --type merge --patch '{"spec":{"storage":{"pvc":{ "claim": "ocs4registry"}}}}'
----
And this command.
