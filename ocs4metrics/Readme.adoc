...
While the documentation recomends using the local-storage provider, we will set up the Prometheus stack to use OCS. By doing so, we will ensure that the Prometheus Pods can move freely between Nodes. In a later section, we will show what this means for performance, by comparing the performance of the default EmptyDir, the recommended local-storage and OCS-backed Prometheus.
