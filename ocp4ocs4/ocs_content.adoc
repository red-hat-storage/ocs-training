...
ip-10-0-170-49.{aws-region}.compute.internal    Ready    worker   77m   v1.14.6+9fb2d5cf9
----
...
cluster-ocs-0ec4-dgwqc-worker-{aws-region}c   1         1         1       1           86m
----
...
Now you will create new *machinesets* and *machines* for OCS4.

[source,role="execute"]
----
ifdef::roadshow[]
curl -s https://lfbf6bao0k.execute-api.eu-central-1.amazonaws.com/machineset-lambda?clusterID=$CLUSTERID | oc apply -f -
endif::[]
ifdef::training[]
...
cluster-ocs-0ec4-dgwqc-workerocs-{aws-region}c-rx4v8   running   {instance-type}   {aws-region}   {aws-region}c   70s
----

You can see that the workerocs *machines* are using are also using the AWS EC2 instance type `{instance-type}`. The `{instance-type}` instance type follows our recommended instance sizing for OCS, 16 cpu and 64 GB mem.
...
cluster-ocs-0ec4-dgwqc-workerocs-{aws-region}c   1         1         1       1           8m25s
----
...
ip-10-0-170-49.{aws-region}.compute.internal    Ready    worker   128m    v1.14.6+9fb2d5cf9
----
...
// ----
// ifdef::roadshow[]
// oc apply {{ HOME_PATH }}/support/ocslab_deploy-with-olm.yaml
// endif::[]
// ifdef::training[]
// oc apply -f http://bit.ly/ocs-olm-training
// endif::[]
// ----
...
image::OCP-installed-operators.png[Openshift showing the installed operators in namespace openshift-storage]

Click on `Openshift Container Storage Operator` to get to the OCS configuration screen.

.OCS configuration screen
image::OCS-config-screen-all.png[OCS configuration screen]
...
image::OCS-config-screen-storage-cluster.png[OCS Create Storage Cluster]

A dialog box will come up next.

.OCS create a new storage cluster
image::OCS-config-screen-new.png[OCS create a new storage cluster]
...
NOTE: The NooBaa pod used the `ocs-storagecluster-ceph-rbd` storage class for creating a PVC for mounting to its `db` container.

=== Using the Rook-Ceph toolbox to check on the Ceph backing storage

Starting with OpenShift Container Storage 4.3 the deployment of a *toolbox* can be configured using the built-in CRs.

Patch the `OCSInitialization ocsinit` using the following command line:
...
Feel free to download `{github-raw-url}/ocp4ocs4/configurable-rails-app.yaml` to check on the format of this template. Search for `STORAGE_CLASS` in the downloaded content.
...
ifdef::roadshow[]
oc new-app {{ HOME_PATH }}/support/ocslab_rails-app.yaml -p STORAGE_CLASS=ocs-storagecluster-ceph-rbd -p VOLUME_CAPACITY=5Gi
endif::[]
ifdef::training[]
curl {github-raw-url}/ocp4ocs4/configurable-rails-app.yaml | oc new-app -p STORAGE_CLASS=ocs-storagecluster-ceph-rbd -p VOLUME_CAPACITY=5Gi -f -
endif::[]
----
...
----
ifdef::roadshow[]
oc apply {{ HOME_PATH }}/support/ocslab_cluster-monitoring-noinfra.yaml
endif::[]
ifdef::training[]
oc apply -f {github-raw-url}/ocp4ocs4/cluster-monitoring-config.yaml
endif::[]
----
.Sample output:
----
...
[source,role="execute"]
----
oc apply {{ HOME_PATH }}/support/ocslab_cluster-monitoring-withinfra.yaml
----
.Sample output:
----
...
----
ifdef::roadshow[]
oc apply {{ HOME_PATH }}/support/ocslab_obc-app-example.yaml
endif::[]
ifdef::training[]
curl -s {github-raw-url}/ocp4ocs4/obc_app_example.yaml | oc apply -f -
endif::[]
----
.Example output:
----
...
cluster-ocs-0ec4-dgwqc-workerocs-{aws-region}c   1         1         1       1           3h50m
----
...
[subs=attributes+]
----
oc scale -n openshift-machine-api --replicas=2 machineset.machine.openshift.io/cluster-ocs-0ec4-dgwqc-workerocs-{aws-region}a
...
machineset.machine.openshift.io/cluster-ocs-0ec4-dgwqc-workerocs-{aws-region}c scaled
----
...
[subs=attributes+]
----
ip-10-0-133-99.{aws-region}.compute.internal
ip-10-0-158-153.{aws-region}.compute.internal
ip-10-0-160-200.{aws-region}.compute.internal
----
...
[subs=attributes+]
----
oc label nodes ip-10-0-131-209.{aws-region}.compute.internal cluster.ocs.openshift.io/openshift-storage=
...
image::OCS-Storage-Cluster-overview-reachit.png[]
...
rook-ceph-osd-prepare-ocs-deviceset-2-1-glsgj-x4k7t               Succeeded   ip-10-0-158-153.{aws-region}.compute.internal
----
...
Disconnect from the Ceph toolbox pod.
...
Must-gather is a tool for collecting data about the currently running Openshift cluster. It loads a predefined set of containers that execute multiple programs and dump it on the local workstations filesystem in unique must-gather directories.
...
[source,role="execute"]
----
oc adm must-gather --image=registry.redhat.io/ocs4/ocs-must-gather-rhel8:v4.3
----
