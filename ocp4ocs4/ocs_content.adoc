...
ifdef::roadshow[]
NOTE: If the lambda function failed, go to <<Use a local YAML file as Machineset>> and try the fallback in that section
endif::[]
...
// ----
// ifdef::roadshow[]
// oc apply -f {{ HOME_PATH }}/support/ocslab_deploy-with-olm.yaml
// endif::[]
// ifdef::training[]
...
ifdef::roadshow[]
[appendix]
== Use a local YAML file as Machineset

This section will explain how to add the OCS machineset without using the lambda function.

[source,role="execute"]
----
oc apply -f /opt/app-root/src/support/ocslab_cluster-workerocs.yaml
----
endif::[]

Afterwards, go back to section <<Scale OCP cluster and add 3 new nodes>>
