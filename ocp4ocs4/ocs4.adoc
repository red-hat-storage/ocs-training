...
[.lead]
It is a battle-tested software defined storage (SDS) solution that has been available as a storage backend fpr OpenStack and Kubernetes for quite some time.
...
[NOTE]
====
A Ceph pool has no size and is able to consume the space available any OSDs where its PGs are created. A Placement Group or PG belongs to only one pool.
====
