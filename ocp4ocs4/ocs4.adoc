...
You will be provided with the location of a manifest that is needed to make the OCS 4 *Operator* visible for installation in OperatorHub. Save the content of the manifest to filename deploy-with-olm.yaml. 

NOTE: This manifest file will not be needed when OCS 4 is released. 
...
On the next screen make sure the settings are as shown in this figure. Make sure to change to `A specific namespace on the cluster` and chose namespace `openshift-storage`.
