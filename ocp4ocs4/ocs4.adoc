...
image::imgs/rook_diagram_3.png[Showing the Rook architecture]

.Ceph deployed on OpenShift using Rook
image::imgs/Rook_diagram_4.png[Ceph on Openshift using Rook]
...
You can see that the workerocs *machines* are using a different AWS EC2 instance type `m5.4xlarge`. The `m5.4xlarge` instance type follows our recommended instance sizing for OCS and provides the best performance/price ratio. Now you want to see if our new *machines* are added to the OCP cluster.
