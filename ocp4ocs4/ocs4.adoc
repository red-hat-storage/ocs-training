...
Click on the *Alerting* item to open the Alert window.

.OCP Alerting Homepage
image::imgs/metrics-alertinghomepage.png[OCP Alerting Homepage]

You can display the alerts in the main window by status. To do so you must highlight the statuses you want to display. The statuses are:
...
NOTE: An alter transitions from `Pending` to `Firing` state if it persists for more than 10 minutes.

.OCP Alerting Status Filtering
image::imgs/metrics-alertingstatusfilter.png[OCP Alert Status Filtering]

You can also filter alerts by name using the *Filter* area on the top right of the window to search for a particular alert or set of alerts.

.OCP Alerting Name Filtering
image::imgs/metrics-alertingnamefilter.png[OCP Alert Name Filtering]

Through the 3 dot icon on the right hand side of each alert line you have access to a contextual menu to either view the alert definition or to silence the alter.
...
NOTE: If desired, you can click the actual existing Prometheus query embedded in the alert. Doing so will take you to the *Metrics* page where you will be able to execute the alter and to test updates to the alert.
...
Use the query field to either enter the formula of your choice or to search for metrics by name. The metrics available will let you query both OCP related information or Ceph related information. The queries can be simple or complex using the Prometheus query syntax and all its available functions.
