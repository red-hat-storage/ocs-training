...
This section covers the different tools available with OPCS 4.2 when it comes to monitoring the environment. This section relies on the existing UI.

Red Hatters familiar with OCP will already familiar with this section but for those not yet familiar with the technology or do not deal with it on a regular basis it will be a good bootstrap.

The tools are accessible through the left menu pane of the main window by clicking on the *Monitoring* menu item. Once the *Monitoring* item is expanded you will have access to 3 choices:
...
Click on the *Metrics* item to open the Prometheus integrated query window as illustrated below

.OCP UI Metrics Homepage
image::imgs/metrics-queryfield.png[OCP Monitoring Metrics Homepage]

Use the query field to either enter the formula of your choice or to search for metrics by name. The metrics available will let you query both OCP related information or Ceph related information. The queries can be simple or complex queries using the Prometheus query syntax and all its available functions.

Let's start testing a simple query example and enter the following in the query field. When you are done typing, simply hit kbd:[Enter]
...
Then let's try a more relevant query example and enter the following in the query field. When you are done typing, simply hit kbd:[Enter]
...
All OCP metrics are also available through the integrated Metrics window. Feel free to try with any of the OCP related metrics such as `process_cpu_total_seconds` for example.
...
Click on the *Dashboards* item in the OCP UI left pane to open the Grafana dashboard page. Upon your first visit to the Grafana page, you will be prompted to use *OpenShift Login* credentials.
...
image::imgs/metrics-grafanadashboard.png[Grafana Dashboard]
