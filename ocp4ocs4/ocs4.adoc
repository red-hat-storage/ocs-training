...
[appendix]
== Ceph 101 - An introduction to Ceph concepts and terminology
...
The Ceph project has a long history behind it and is not as young as ones may think. It is a battle-tested software defined storage
solution.
...
clients present within the IT infrastructure to get access to the data. The entire Ceph architecture is resilient and does not present
...
.Monitors
The Monitors maintain the cluster map and state and provide distributed decision-making while configured in an odd number to prevent split-brain situations.
The Monitors are not in the data-path and do not serve IO requests to and from the clients.

.OSDs
One OSD is typically deployed for each local block devices and the native scalable nature of Ceph allows for tghousands of OSDs to be part of the cluster.
...
<1> Balancer module
<2> Placement Group auto-scaler module
<3> Dashboard module
<4> RESTful module
<5> Prometheus module
<6> Zabbix module
...
.RADOS as it stands
image::imgs/ceph101-rados.png[RADOS Overview,align="center"]

Ceph was designed to provides the IT environment with all the necessary access methods.

.Access Methods
image::imgs/ceph101-differentstoragetypes.png[Ceph Access Modes,align="center"]

Librados allows developers to code natively against the native Ceph cluster API for maximum efficiency combined with a small footprint. The Ceph native
API offers different wrappers such as C, C++, Python, Java, Ruby, Erlang, Go and Rust.

.Application Native Object API
image::imgs/ceph101-librados.png[librados,align="center"]

This access method is used in Red Hat Enterprise Linux or Openshift 3 and 4. This mode currently does not allow leveraging the
RBD Mirroring feature. RBD are typically used for RWO claims in OCP3 and 4.

.Block Access (RADOS Block Device or RBD)

image::imgs/ceph101-krbd.png[Kernel based RADOS Block Device,align="center"]
...
This mode will allow us to leverage RBD Mirroring in future version. RBD are typically used for RWO claims in OCP3 and 4

image::imgs/ceph101-librbd.png[Userspace RADOS Block Device,align="center"]

This method allows clients to jointly access a shared POSIX compliant filesystem. The client initally contacts the Meta Data Server to obtain
the location of the object(s) for a given inode and then communicates directly with an OSD to perform the final IO request. CephFS is typically
used for RWX claims but can also be used to support RWO claims.

.File Access (Ceph Filesystem or CephFS)

image::imgs/ceph101-cephs.png[Kernel Based CephFS Client,align="center"]

This access method offers support for the Amazon S3 and Openstack Swift support on top of a Ceph cluster. The Openshift Conatiner Storage MultiCloud
Gateway can leverage the RADOS to support Object Bucket Claims.

.Amazone S3 or Openstack Swift (Ceph RADOS Gateway)

image::imgs/ceph101-rgw.png[S3 and Swift Support,align="center"]
...
and only one hash bucket know as a Placement Group (aka PG).

.Ceph Data Placement
image::imgs/ceph101-crushfromobjecttoosd.png[From Object to OSD,align="center"]

CRUSH is the central point of configuration for the topolgy of the cluster. It offers a pseudo-random placement algorithm to distribute the objects across
...
.From Object to OSD
image::imgs/ceph101-thefullpicture.png[From Object to OSD Detailed,align="center"]
...
The cluster will be separated into logical partitions, knwown as a pool. Each pool has the following properties that can be adjusted:

<1> Each pool has a name
<2> A number of PGs to distribute the objects across the OSDs
<3> A CRUSH rule to determine the mapping of the PGs for this pool
<4> A type of protection (Replication or Erasure Coding)
<5> The parameters associated with the type of protection
<6> Various flags to influence the behaviour of the cluster

.Ceph Data Protection
image::imgs/ceph101-dataprotection.png[Repliacted Pools vs Erasure Coded Pools,align="center"]
...
a 5GB S3 object stored through the RADOS Gateway via the MultiCloud Gateway will be divided in multiple objects.

Out of the box, each access method uses an object size of 4MB. As an example, the following schematic shows how a 32MB RBD (Block Device) supporting a RWO PVC will
actually be scattered throughout the cluster.

.Data Distribution
image::imgs/ceph101-rbdlayout.png[RADOS Block Device Layout,align="center"]
