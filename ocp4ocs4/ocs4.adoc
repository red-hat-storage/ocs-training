...
This hands-on workshop is for both system administrators and application developers interested in learning how to deploy and manage OpenShift Container Storage (OCS). In this lab you will be using OpenShift Container Platform (OCP) 4.x and the OCSfootnote:OCS[Openshift Container Storage] operator to deploy Ceph and the Multi-Cloud-Gateway (MCG) as a persistent storage solution for OCPfootnote:OCP[OpenShift Container Platform] workloads. This instruction was developed using the RHPDS `OpenShift 4.2 Workshop`.
...
In this section, you will validate the OCPfootnote:OCP[] environment has 3 master and 3 worker nodes before increasing the cluster size by 3 worker nodes. The `NAME` of your OCPfootnote:OCP[] nodes will be different than shown below.
...
Now you are going to add 3 more OCPfootnote:OCP[] compute nodes to cluster using *machinesets*.
...
This will show you the existing *machinesets* used to create the 3 worker nodes in the cluster already. There is a *machineset* for each AWSfootnote:AWS[Amazon Web Services] AZ (us-east-1a, us-east-1b, us-east-1c). Your *machinesets* `NAME` will be different than below. 
...
Now you are ready to create your 3 new OCPfootnote:OCP[] worker nodes using these modified *machinesets*.
...
You can see that the workerocs *machines* are using a different AWSfootnote:AWS[] EC2 instance type `m4.4xlarge`. The `m4.4xlarge` instance type follows our recommended instance sizing for OCSfootnote:OCS[] and provides the best performance/price ratio. Now you want to see if our new *machines* are added to the OCPfootnote:OCP[] cluster.
...
Now check to see that you have 3 new OCPfootnote:OCP[] worker nodes. The `NAME` of your OCP nodes will be different than shown below.
...
In this section you will be using three of the worker OCPfootnote:OCP[] nodes to deploy OCSfootnote:OCS[] 4. For this you will be using a manifest file, which adds multiple items to your OCPfootnote:OCP[] cluster as shown below. Using the manifest for deployment is temporary until OCSfootnote:OCS[] 4 is generally available (GA). At that time the OCSfootnote:OCS[] will be installed from OperatorHub in OCPfootnote:OCP[] 4 instead of using this manifest.

- The `openshift-storage` namespace
- The `local-storage` namespace
- Operator groups and sources for the OCSfootnote:OCS[] and local-storage operators
- An OCSfootnote:OCS[] subscription
...
The resource `csv` is a shortened word for `clusterserviceversions.operators.coreos.com`. Please wait until the operator `PHASE` changes to `Succeeded` - this will mark that the installation of your operators was successful. Reaching this state can take several minutes.
...
Now switch over to your Openshift Web console. You can get your URL by issuing command below to get the `console` route. Put this URL in a browser tab. You will use the same username and password you used to login and use the oc CLI to login to the OCPfootnote:OCP[] `console`.
...
Click on `Openshift Container Storage Operator` to get to the OCSfootnote:OCS[] configuration screen.

.OCS configuration screen
image::imgs/OCS-config-screen.jpg[OCS configuration screen]

On the OCSfootnote:OCS[] configuration screen, scroll down to the box labelled `Storage cluster` and click on `Create Instance`.

.OCS create a new storage cluster
image::imgs/OCS-config-screen-new.png[OCS create a new storage cluster]

To select the worker nodes using the m4.4xlarge EC2 instance type that you created earlier in the lab, you can find them by searching for `role=storage-node`. It would be a good practice to add a unique label to OCPfootnote:OCP[] nodes (i.e., role=storage-node) that are to be used for creating the `Storage Cluster` so they are easy to find in list of OCPfootnote:OCP[] nodes.
...
include::workarounds/fix-pool-crush-rules.adoc[]
...
You can now also check the status of your storage cluster with the OCSfootnote:OCS[] specific *Dashboards* that are included in your Openshift Web Console. You can reach this by clicking on `Home` on your left navigation bar, then selecting `Dashboards` and finally clicking on `Persistent Storage` on the top navigation bar of the content page.
...
OCSfootnote:OCS[] ships with a *Dashboard* for the Object Store service as well. From within the *Dashboard* menu click on the `Object Service` on the top navigation bar of the content page.
...
Once this is all healthy, you will be able to use the three new `StorageClasses` created during the OCSfootnote:OCS[] 4 Install:
...
Since the Rook-Ceph *toolbox* is not shipped with OCSfootnote:OCS[], we need to deploy it manually. For this, we can leverage the upstream `toolbox.yaml` file, but we need to modify the namespace as shown below.
...
After installing OCSfootnote:OCS[], it is best practice to change the default *storage class* from AWSfootnote:AWS[] gp2 to our new OCS-backed storage class `ocs-storagecluster-ceph-rbd`.
...
In this section the `ocs-storagecluster-ceph-rbd` *storage class* will be used by an OCPfootnote:OCP[] application + database *deployment* to create RWO (ReadWriteOnce) persistent storage. The persistent storage will be a Ceph RBD (RADOS Block Device) volume (object) in the Ceph pool `ocs-storagecluster-cephblockpool`.
...
.Example output:
----
INFO[0000] CLI version: 2.0.1
INFO[0000] noobaa-image: noobaa/noobaa-core:5
INFO[0000] operator-image: noobaa/noobaa-operator:2.0.1
INFO[0000] Namespace: noobaa201
INFO[0000]
...
INFO[0002] ✅ Exists: Namespace "noobaa201"
INFO[0003] ✅ Exists: ServiceAccount "noobaa"
INFO[0003] ✅ Exists: Role "noobaa"
INFO[0003] ✅ Exists: RoleBinding "noobaa"
INFO[0003] ✅ Exists: ClusterRole "noobaa201.noobaa.io"
INFO[0003] ✅ Exists: ClusterRoleBinding "noobaa201.noobaa.io"
INFO[0003] ✅ Exists: Deployment "noobaa-operator"
INFO[0003]
INFO[0003] System Status:
INFO[0004] ✅ Exists: NooBaa "noobaa"
INFO[0004] ✅ Exists: StatefulSet "noobaa-core"
INFO[0004] ✅ Exists: Service "noobaa-mgmt"
INFO[0004] ✅ Exists: Service "s3"
INFO[0004] ✅ Exists: Secret "noobaa-server"
INFO[0004] ✅ Exists: Secret "noobaa-operator"
INFO[0005] ✅ Exists: Secret "noobaa-admin"
INFO[0005] ✅ Exists: StorageClass "noobaa201.noobaa.io"
INFO[0005] ✅ Exists: BucketClass "noobaa-default-bucket-class"
INFO[0005] ✅ Exists: BackingStore "noobaa-default-backing-store"
INFO[0005] ✅ Exists: CredentialsRequest "noobaa-cloud-creds"
INFO[0005] ✅ Exists: PrometheusRule "noobaa-prometheus-rules"
INFO[0006] ✅ Exists: ServiceMonitor "noobaa-service-monitor"
INFO[0006] ✅ Exists: PersistentVolumeClaim "db-noobaa-core-0"
INFO[0006] ✅ System Phase is "Ready"
...
ExternalDNS : [https://a4fd12697f4c211e99e8c026ac919a8b-123456789.us-east-2.elb.amazonaws.com:443]
ExternalIP  : []
NodePorts   : [https://10.0.163.235:32200]
InternalDNS : [https://noobaa-mgmt.openshift-storage:443]
InternalIP  : [https://172.30.232.67:443]
PodPorts    : [https://10.131.0.180:8443]
...
password: By4Jk3Z8t6vOvcg/PFVolQ==
system: noobaa
...
ExternalDNS : [https://a4fd4c63cf4c211e99e8c026ac919a8b-123456789.us-east-2.elb.amazonaws.com:443]
ExternalIP  : []
NodePorts   : [https://10.0.163.235:30010]
InternalDNS : [https://s3.openshift-storage:443]
InternalIP  : [https://172.30.82.63:443]
PodPorts    : [https://10.131.0.180:6443]

#------------------#
#- S3 Credentials -#
#------------------#

AWS_ACCESS_KEY_ID: P95HA2aGGLtpfabcZrpR
AWS_SECRET_ACCESS_KEY: 922pAbpN4d55555N1V8QO33MwnlbIvgO4jjkGiy3
...
noobaa-default-backing-store   aws-s3   noobaa-backing-store-8858a19c-94ad-44ec-bbe3-404fdf22bdfa   Ready   13m51s
...
noobaa-default-bucket-class   {Tiers:[{Placement: BackingStores:[noobaa-default-backing-store]}]}   Ready   13m52s
...
metadata:
  creationTimestamp: "2019-10-22T12:06:54Z"
  finalizers:
  ...
  resourceVersion: "23233576"
  selfLink: /apis/objectbucket.io/v1alpha1/namespaces/openshift-storage/objectbucketclaims/test21obc
  uid: 701800e5-f4c4-11e9-b850-0a40f81e875e
spec:
  ObjectBucketName: obc-openshift-storage-test21obc
  bucketName: test21obc-87af5dac-ca7b-4517-9aba-193767b6137f
...
apiVersion: v1
data:
  AWS_ACCESS_KEY_ID: SE1rMUkyYWFmYUppWURRajV2T0U=
  AWS_SECRET_ACCESS_KEY: dStQZTNkalZ4eEphd3JGTUJuRWlDd2t0Q1BReEk3MkxlUi9QdDJBTg==
kind: Secret
metadata:
  creationTimestamp: "2019-10-22T12:06:54Z"
  finalizers:
  ...
  - apiVersion: objectbucket.io/v1alpha1
    ...
    name: test21obc
    uid: 701800e5-f4c4-11e9-b850-0a40f81e875e
  resourceVersion: "23233571"
  selfLink: /api/v1/namespaces/openshift-storage/secrets/test21obc
  uid: 7037b227-f4c4-11e9-9e8c-026ac919a8be
type: Opaque
----
...
apiVersion: v1
data:
  BUCKET_HOST: 10.0.163.235
  BUCKET_NAME: test21obc-87af5dac-ca7b-4517-9aba-193767b6137f
  BUCKET_PORT: "30010"
  BUCKET_REGION: ""
  BUCKET_SUBREGION: ""
kind: ConfigMap
metadata:
  ...
  - apiVersion: objectbucket.io/v1alpha1
    ...
    name: test21obc
    uid: 701800e5-f4c4-11e9-b850-0a40f81e875e
  resourceVersion: "23233572"
  selfLink: /api/v1/namespaces/openshift-storage/configmaps/test21obc
  uid: 70393658-f4c4-11e9-9e8c-026ac919a8be
----
...
Adding storage to OCSfootnote:OCS[] adds capacity and performance to your already present cluster. For this you can select to either add more storage worker nodes, or leverage the already present nodes to add more storage capacity.

=== Add storage worker nodes

This section will explain how one can add more worker nodes to the present storage cluster. Afterwards follow the next sub-section on how to extend the OCSfootnote:OCS[] cluster to provision storage on these new nodes.

To add more nodes, we could either add more machinesets like we did before, or scale the already present OCSfootnote:OCS[] machinesets. For this training, we will spawn more workers by scaling the already present OCSfootnote:OCS[] worker instances up:
...
Now we have the new instances prepared for extending the cluster, proceed to the next chapter to provision storage on these empty instances, the OCSfootnote:OCS[] operator will pick prefer the empty instances for new storage.

=== Add storage capacity

In this section we will add storage capacity and performance to the configured OCSfootnote:OCS[] worker nodes. If you have followed the previous section you should now have 6 OCSfootnote:OCS[] nodes, otherwise you have three worker nodes, which is fine too.

To add storage, go to the Openshift Web Console and follow the steps to reach the OCSfootnote:OCS[] storage cluster overview:
...
In the new dialog you can set the requested additional (usable) capacity and the storage class. On AWSfootnote:AWS[], the storage class should be set to `gp2`.

NOTE: The effectivly provisioned capacity will be three times as much as you put into the `Requested Capacity` field, because OCSfootnote:OCS[] uses a replica count of 3.
...
You can now see that there are new OSD pods and if you previously added new ndoes that they use the new OCSfootnote:OCS[] worker nodes:
...
This is everything that you need to do to extend the OCSfootnote:OCS[] storage.
...
The OCSfootnote:OCS[] team has released its own image for the must-gather tool that runs storage specific commands.
...
Or like this for OCSfootnote:OCS[] specific insights:
