...
* Verify that the nodes labeled for OCS exist in three distinct racks or zones after applying the OCS label.
* Verify that the number of nodes in each rack (VMware or bare metal) or zone (AWS) is equal. For example, if you labeled 3 workers with the OCS label there should be one node in each rack or zone.

[source,role="execute"]
----
oc get nodes -L failure-domain.kubernetes.io/zone,failure-domain.kubernetes.io/rack -l cluster.ocs.openshift.io/openshift-storage=''
----
