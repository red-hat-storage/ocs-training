...
This process should be followed when an OSD *Pod* is in an `Error` state and the root cause is the underlining storage device. 
...
In the case where the data can migrated off the OSD in a `Error` state, you will want to wait until all *PGs* are `active+clean`.
...
----
192 pgs: 192 active+clean; 380 MiB data, 1015 MiB used, 1.5 TiB / 1.5 TiB avail; 1.2 KiB/s rd, 59 KiB/s wr, 8 op/s
----

Now this OSD needs to be removed from the Ceph `Crush Map`.
...
Now the device name needs to be identified. In this example the device name is `sdb`.
...
[source,role="execute"]
----
oc describe -n openshift-storage pvc ocs-deviceset-1-0-959rp | grep Mounted
----
.Example output
----
...
First step is to login machine/node with failed drive and record the /dev/disk/by-id/{id} that is to be replaced. In this example the OCP node is `compute-2`.
...
After the new `/dev/disk/by-id/{id}` is available a new disk entry (/dev/disk/by-id/<new_id>) can be added to the localvolume local-block CR.  
...
Logon to OCP node with failed device and remove the old symlink. Validate it is removed before proceeding.
...
Delete the *PV* for failed device. This *PV* name was identified in an earlier step. In this example the *PV* name is `local-pv-d9c5cbd6`.
...
Next step is to delete the deployment for the failed device that was scaled to `replicas=0` in and earlier step. 

[source,role="execute"]
----
oc get deployments | grep osd
----
.Example output
----
...
Now that the deployment and all other associated OCP and Ceph resources for the failed device are deleted or removed, the new OSD can be created. This is done by restarting the `rook-ceph-operator`.
...
Last step is to validate there is a new OSD, that Ceph is healthy, and that a successful replacement shows in the *OCP Web Console* Dashboards. 
...
There now is a new OSD with `rook-ceph-osd-0` name that was created in last 5 minutes for this example. Next step is to login to Ceph and see if the cluster is healthy.
...
Now logon to *OCP Web Console* and view the storage Dashboard.
