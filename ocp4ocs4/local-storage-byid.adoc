...
* Verify that the nodes labeled for OCS exist in three distinct racks or zones, or that there are no zone or rack topology labels.
* Verify that the number of nodes in each rack or zone is equal (eg. one in each, two in each).
