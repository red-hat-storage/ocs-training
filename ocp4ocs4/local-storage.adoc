...
Reference <<Using the LSO LocalVolume Custom Resource to Create PVs>> for more commands to validate creation of `local-storage` *Pods* and *PVs* created from storage devices listed under `devicePaths` in the LocalVolume CR(s).
...
[NOTE]
====
If your OCP deployment has the `thin` *StorageClass* available for creating and attaching VMDKs to OCP nodes this can be used in place of `localfile` above.
====
...
To find the storage device names use a similar method as done for AWS and VMware environments by logging on to the OCP worker nodes with the OCS label (see <<Prerequisites>>) and issuing a `lsblk`. By inspecting the results of this command you will know which devices are available and their size. See <<Finding Available Storage Devices>> for more details.

For Monitor storage a Filesystem *StorageClass* does not need to be created as it was for VMware. In the *StorageCluster* CR instead *monDataDirHostPath* will be used and set to the path of /var/lib/rook for Monitor storage.

[NOTE]
====
If desired you could use an available raw block device (e.g., /dev/sdb) and create a LocalVolume CR with Filesystem volume mode resulting in the  `localfile` *StorageClass*. Use `localfile` as shown in the *StorageCluster CR example for VMware for Monitor storage. The downside of this approach is the Monitor storage requirement is a minimum of 10 GB and most storage devices are larger, thereby wasting space.
====
