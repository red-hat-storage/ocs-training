...
image::imgs/OCS4-OCP-OperatorHub-Filter.png[OCP OperatorHub Filter]
...
image::imgs/OCS4-OCP-OperatorHub-Subscribe.png[OCP OperatorHub Subscribe]
...
image::imgs/OCP4-installed-operators.png[Openshift showing the installed operators in namespace openshift-storage]
...
image::imgs/OCP4-LSO-filter.png[OperatorHub LSO Operator filter]
...
image::imgs/OCS4-OCP-OperatorHub-LSO-Subscribe.png[OCP OperatorHub Subscribe]
...
The Local Storage Operator (LSO) has been successfully installed. Now more on to creating local persistent volumes (PVs) on the storage nodes using LocalVolume Custom Resource (CR) files.
...
And finally we should have an additional *StorageClass* as a result of creating this LocalVolume CR. This *StorageClass* will be used to create *PVCs* in the next step of creating a Storage Cluster. 
...
The next sections will detail how to create and validate the OCS Storage Cluster using Amazon EC2, VMware, and bare metal local storage devices.
