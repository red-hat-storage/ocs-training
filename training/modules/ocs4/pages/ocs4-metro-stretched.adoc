...
Example *StorageCluster* CR for `Metro DR stretch cluster`. For each set of 4 OSDs increment the `count` by 1. Creating the ODF `StorageCluster` using `oc` CLI is a replacement for the method described earlier in this document using the *OpenShift Web Console` UI.
