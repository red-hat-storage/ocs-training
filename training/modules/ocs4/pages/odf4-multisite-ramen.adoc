...
The *Cluster Network Operator* injects SSL certificates across any namespace when an empty *ConfigMap* is created with the config.openshift.io/inject-trusted-cabundle=true label. To cause this injection to happen for certificates in `user-ca-bundle` save the following YAML file to filename `cm-injector-crt.yaml`.
...
[source,role="execute"]
----
oc create -f cm-injector-crt.yaml
----
.Example output.
----
configmap/trusted-ca created
----

NOTE: Create this resource on the *Primary managed cluster* and the *Secondary managed cluster*.

Now we need to update the `ramen-dr-cluster-operator` *Deployment* with the trusted certificate. Start by editing the *CSV* in the project with this *Deployment*.

[source,role="execute"]
----
oc edit -n openshift-dr-system csv odr-cluster-operator.v4.9.0
----
...
Add the new `mountPath` under *volumeMounts* and the new `configMap` under *volumes*. Save the changes in *CSV* `odr-cluster-operator.v4.9.0` and exit.

NOTE: This step needs to be done on the *Primary managed cluster* and the *Secondary managed cluster*.

To validate the change has been made successfully run the following command for the new `volumeMounts` in the `ramen-dr-cluster` *Pod*:

[source,role="execute"]
----
oc get pod -l app=ramen-dr-cluster -o jsonpath='{.items[0].spec.containers[0].volumeMounts[1]}{"\n"}'
----
Example output.
----
{"mountPath":"/etc/pki/ca-trust/extracted/pem","name":"trusted-ca","readOnly":true}
----

To validate the change has been made successfully run the following command for the new `volumes` in the `ramen-dr-cluster` *Pod*:

[source,role="execute"]
----
oc get pod -l app=ramen-dr-cluster -o jsonpath='{.items[0].spec.volumes[1]}{"\n"}'
----
Example output.
----
{"configMap":{"defaultMode":420,"items":[{"key":"ca-bundle.crt","path":"tls-ca-bundle.pem"}],"name":"trusted-ca"},"name":"trusted-ca"}
----

=== Retrieve MCG keys and external S3 endpoint

First check the MCG is installed on *Primary managed cluster* and the *Secondary managed cluster* and that the `Phase` is *Ready*.
