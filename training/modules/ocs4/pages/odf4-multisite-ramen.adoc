...
[source,role="execute"]
----
oc create -f ocs-catalogsource.yaml -n openshift-storage
----
.Example output.
----
...
The block volumes that are created with mirroring enabled must to be created with a new *StorageClass* that has additional `imageFeatures` required for faster image replication between managed clusters. The new features are _exclusive-lock_, _object map_, and _fast-diff_ when compared to the *ODF* default `ocs-storagecluster-ceph-rbd` *StorageClass* for block volumes.
