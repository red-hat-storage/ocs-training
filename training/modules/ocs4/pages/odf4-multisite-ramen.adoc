...
.Example output.
----
configmap/user-ca-bundle-secondary created
----
...
.Example output.
----
configmap/user-ca-bundle-primary created
----

IMPORTANT: The *Hub cluster* needs to verify access to the object buckets using the *DRPolicy* resource. Therefore the same *ConfigMaps*, `cm-primary-crt.yaml` and `cm-secondary-crt.yaml`, need to be created on the *Hub cluster*.
...
The `odrbucket` *OBC* exact bucket name also needs to be found.
