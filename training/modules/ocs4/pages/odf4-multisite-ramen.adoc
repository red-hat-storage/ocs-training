...
This is a controller that will be installed from OCP *OperatorHub* on the *Hub cluster*. The job of this `Multicluster Orchestrator` controller, along with the `MirrorPeer` Custom Resource, is to do the following:

* Create a bootstrap token and exchanges this token between the managed clusters.
* Enable mirroring for the default `CephBlockPool`.
* Create an object bucket (using MCG) on each managed cluster for *PVC* and *PV* metadata.
* Create a *Secret* for each new object bucket that has the keys for bucket access on the *Hub cluster* in the `openshift-dr-system` project. 
...
Validate that the *VolumeReplicationClass* is created on each managed cluster.
...
Validate there is a new *OBC* and corresponding *OB* in each managed cluster in the `openshift-storage` namespace.
...
The *OBC* and *Secrets* are witten in the *ConfigMap* `ramen-hub-operator-config` on the *Hub cluster*.
...
----
    ...
    - s3Bucket: odrbucket-21eb5332f6b6
      s3CompatibleEndpoint: https://s3-openshift-storage.apps.perf2.chris.ocs.ninja
    ...
    - s3Bucket: odrbucket-21eb5332f6b6
      s3CompatibleEndpoint: https://s3-openshift-storage.apps.perf1.chris.ocs.ninja
...
ODR uses *DRPolicy* resources (cluster scoped) on the ACM hub cluster to deploy, failover, and relocate, workloads across managed clusters. A *DRPolicy* requires a set of two clusters, which are peered for storage level replication and `CSI` *VolumeReplication* is enabled.
...
Save the following YAML (below) to filename drpolicy.yaml after replacing *<cluster1>* and *<cluster2>* with the correct names of your managed clusters in *ACM*. Replace *<unique_value_1> * and *<unique_value_2>* with any values as long as they are unique (i.e., east and west).
...
IMPORTANT: The *DRPolicy* scheduling interval *_must_* match that configured in the *VolumeReplicationClass* resource.
...
== Enable automatic install and configuration for managed clusters
