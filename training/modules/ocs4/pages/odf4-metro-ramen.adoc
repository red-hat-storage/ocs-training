...
In order to failover the OpenShift cluster where the application is currently running must have all applications `fenced` from communicating with the external *ODF* storage. This is required to prevent simultaneous writes to the same persistent volume from both managed clusters. 
...
The IP addresses for all nodes will be added to the *NetworkFence* example resource as shown below.
...
NOTE: NetworkFence is a cluster-scoped custom resource. The creation of NetworkFence for a list of IP addresses will add a network fence, and its deletion will undo the operation.
...
IMPORTANT: The *NetworkFence* must be created from the opposite managed cluster from where the application is currently running prior to `failover`.
...
[source,role="execute"]
----
export NETWORKFENCE=network-fence-ocp4perf1
...
In order for the *NetworkFence* status of `Fenced` to be known to the ODR HUB operator the *DRPolicy* must be modified for `fenced` cluster. Edit the *DRPolicy* and change `<cluster1>` (example ocp4perf1) from `Unfenced` to `Fenced`.
...
spec:
  drClusterSet:
  - clusterFence: Fenced  ## <-- Modify from Unfenced to Fenced
    name: ocp4perf1
    region: metro
    s3ProfileName: s3-primary
  - clusterFence: Unfenced
