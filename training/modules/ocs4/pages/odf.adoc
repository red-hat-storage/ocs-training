...
image::OCS-Pods-Diagram.png[OpenShift Data Foundation components]
...
ip-10-0-234-166.us-east-2.compute.internal  Ready    worker   4d4h   v1.23.5+012e945
----
...
ip-10-0-209-53.us-east-2.compute.internal    Ready    worker   14m    v1.23.5+012e945
----
...
ip-10-0-209-53.us-east-2.compute.internal    Ready    worker   15m   v1.23.5+012e945
----
...
rook-ceph-operator-658579dd69-768xd                1/1     Running   0          8m13s
----
...
image::OCS4-OCP4-View-Operator.png[Create storage system in openshift-storage namespace]

The `Create StorageSystem` screen will display.

.Configure storage system settings
image::OCS4-config-screen-partial1.png[Configure storage system settings]

Select the storage class `gp3-csi` and `Full deployment`, the click *Next*.

.Select capacity and nodes for new storage system
image::OCS4-config-screen-partial2.png[Select capacity and nodes for new storage system]
...
image::OCS4-config-screen-partial3.png[Review and create new storage system]
...
rook-ceph-osd-prepare-f38a232f03043ef21bfcf3161415924b-89gv7      0/1     Completed   0          42s
----
...
Reference the https://access.redhat.com/documentation/en-us/red_hat_openshift_data_foundation/4.11/html/monitoring_openshift_data_foundation/cluster_health#verifying-openshift-data-foundation-is-healthy_rhodf[ODF Monitoring Guide] for more information on how to use the *Dashboards*.
...
[source,role="execute"]
----
oc -n openshift-storage get sc | grep openshift-storage
----
.Example output:
----
ocs-storagecluster-ceph-rbd   openshift-storage.rbd.csi.ceph.com      Delete          Immediate              true                   3m12s
ocs-storagecluster-cephfs     openshift-storage.cephfs.csi.ceph.com   Delete          Immediate              true                   3m12s
openshift-storage.noobaa.io   openshift-storage.noobaa.io/obc         Delete          Immediate              false                  107s
----
...
.Example output:
----
postgresql-1-6hbz2                  1/1     Running     0          4m55s
...
rails-pgsql-persistent-1-hook-pre   0/1     Completed   0          31s
----
...
ocs-deviceset-gp3-csi-2-data-0kck5v   pvc-f7a4522d-36dd-4c12-b2c5-0461d8ab6428   gp3-csi                       vol-0ca1e83cb7e325275
----
...
.Examplet output:
----
csi-vol-209ddf4a-278c-11ed-9219-0a580a83020e
----
...
.Example output:
----
rbd image 'csi-vol-209ddf4a-278c-11ed-9219-0a580a83020e':
	...
	modify_timestamp: Mon Aug 29 11:17:11 2022
----
...
.Example output:
----
Warning: oc apply should be used on resource created by either oc create --save-config or oc apply persistentvolumeclaim/postgresql configured
----
...
.Example Output:
----
--> Found image 52f2505 (4 weeks old) in image stream "openshift/php" under tag "8.0-ubi8" for "openshift/php"
...
pod/thanos-querier-85b555c78d-4vvqt                          6/6     Running   0          78m
----
...
== Create a new OCP application deployment using an Object Bucket
...
NOTE: The `MCG Console` is deprecated in ODF 4.11.

=== Checking MCG status

MCG status can be checked with the NooBaa CLI. You may download the NooBaa CLI 
from the NooBaa Operator releases page: https://github.com/noobaa/noobaa-operator/releases. There is also instructions at <<Install the NooBaa CLI client>>.
...
image::photo-album-select-upload.png[Select Photo and Upload]
...
image::metrics-alertingleftpanemenu.png[OCP Observe Menu choose Alerting]
...
image::metrics-metricsleftpanemenu.png[OCP Observe Menu choose Metrics]
...
[source,role="execute"]
----
oc adm must-gather --image=registry.redhat.io/odf4/ocs-must-gather-rhel8:v4.11
----
...
[source]
----
curl -sLO https://github.com/noobaa/noobaa-operator/releases/download/v5.10.1/noobaa-mac-v5.10.1 ; chmod +x noobaa-mac-v5.10.1 ; sudo mv noobaa-mac-v5.10.1 /usr/local/bin/noobaa
----

.Linux steps
[source]
----
curl -sLO https://github.com/noobaa/noobaa-operator/releases/download/v5.10.1/noobaa-linux-v5.10.1 ; chmod +x noobaa-linux-v5.10.1 ; sudo mv noobaa-linux-v5.10.1 /usr/bin/noobaa
----
