...
[source,yaml]
.StorageCluster CR cluster-service-AWS.yaml using `localblock` storageclass:
----
...
[source,yaml]
.StorageCluster CR cluster-service-AWS-expand.yaml using `localblock` storageclass:
----
apiVersion: ocs.openshift.io/v1
kind: StorageCluster
metadata:
