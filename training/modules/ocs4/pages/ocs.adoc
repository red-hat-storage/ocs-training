...
[source,role="execute"]
----
curl -s https://github.com/red-hat-storage/demo-apps/raw/main/packaged/photo-album.tgz | tar xvz
----

.Example output:
----
x photo-album/
x photo-album/documentation/
x photo-album/app/
x photo-album/images/
x photo-album/demo.sh
[...]
----

Then, run the application startup script which will build and deploy the app to your cluster. 
...
NOTE: Please make sure you follow the continuation prompts. 

Deployment might take up to 5 minutes to complete. 
...
Now that the photo-album application has deployed you can view the *Object Bucket Claim* it created. You can also view the new bucket, *Configmap* and *Secret* using the oc command.

To view the *Object Bucket Claim* used by photo-album run the following:
[source,role="execute"]
----
oc -n demo get obc 
----
...
To view the *Object Bucket* that was created by the object bucket claim above, run the following:
[source,role="execute"]
----
oc get ob 
----
...
The Secret will contain the credentials required for the application to connect and access the new object bucket.

To view the Secret created by the OBC, run the following:
[source,role="execute"]
----
...
As you can see when the new OBC is processed, MCG creates a new bucket, Secret and Configmap which contain all the information required for our application to use the new bucket.

In order to view the details of the Object Bucket Claim, view the top of photo-album/app.yaml.
[source,yaml]
----
---
...
To view exactly how the application uses the information in the new *Secret* and *ConfigMap* have a look at the file photo-album/app.yaml after you have deployed the app. In the deployment template specification section, find 'env:' and you can see how the *ConfigMap* and *Secret* details are mapped to environment variables. 
...
In order to see objects in your new bucket, find your route:
[source,role="execute"]
----
...
Copy and paste this route into a browser tab. 

image::photo-album-select-upload.png[]

Select 1 or more photos, then make sure to upload.

image::photo-album-images.png[]

Then navigate to the MCG console by viewing the *Object Service Dashboard* mentioned previously, and selecting the Multcloud Object Gateway link under System Name:

image::System-Name-MCG-Console.png[]

Login to the MCG Console using your Openshift Kubeadmin credentials. You can navigate to the bucket details by selecting the bucket from the Bucket menu:

image::MCG-Console-photo-album-buckets.png[]

Once in the bucket, select the object tab to view the individual objects create when you uploaded your images:

image::MCG-Console-photo-album-objects.png[]
