...
[start=1]
. *Deploy HasshiCorp Vault.* +
Download HashiCorp Vault and configure it so it can be used by ODF.
. *Install ODF 4.7.* +
Install ODF 4.7 on your OCP cluster.
. *Create your data at rest encrypted Storage Cluster.* +
Deploy your storage cluster pointing to HashICorp vault to provide storage for the generated encryption keys.
. *Deploy an application with PV encryption.* +
Deploy your application over encrypted *PersistentVolumes* and use HashiCorp vault to store the generated encryption keys.
...
== PV encryption without at-rest cluster encryption
