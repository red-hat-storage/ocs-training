...
Given the persistent data is going to be mirrored or replicated from the *primary cluster* to the *secondary cluster* we do not need the `OADP` *Backup* to include this data and therefore set `snapshotVolumes: false`. Also, we do not need the application to build again in this case so `buildconfigs.build` is excluded from the backup operation.
...
<4> The number of OpenShift resources backed up
...
Now check to see that the *PODs* and *PVC* are created correctly in `my-database-app`namespace on *secondary cluster*.
...
----
NAME                                    READY   STATUS      RESTARTS   AGE
pod/postgresql-1-deploy                 0/1     Completed   0          12m
pod/postgresql-1-hrjlh                  1/1     Running     0          12m
pod/rails-pgsql-persistent-1-deploy     0/1     Completed   0          12m
pod/rails-pgsql-persistent-1-hook-pre   0/1     Completed   0          12m
pod/rails-pgsql-persistent-1-lmr4c      1/1     Running     0          11m

NAME                               STATUS   VOLUME                                     CAPACITY   ACCESS MODES   STORAGECLASS                     AGE
persistentvolumeclaim/postgresql   Bound    pvc-ec266c85-6606-44d4-8d47-8a008d513d0e   5Gi        RWO            ocs-storagecluster-ceph-mirror   12m
----
