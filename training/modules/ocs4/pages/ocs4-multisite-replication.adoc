...
The `rbd-mirror` daemon(s) can be created using a custom resource definition (CRD). There must be a `rbd-mirror` daemon or *Pod* created on the *primary cluster* and the *secondary cluster* using this CRD:
...
Before any new OCS volumes are created for replication a new *StorageClass* needs to be created using *CephBlockPool* replicapool that was created in prior section. Also, the `reclaimPolicy` needs to be `Retain` verses `Delete` as it is in the default pool.
...
--------+----------------------+----------+---------+------------+-------------
 public | ar_internal_metadata | table    | userCY2 | 16 kB      | 
 public | articles             | table    | userCY2 | 16 kB      | 
 public | articles_id_seq      | sequence | userCY2 | 8192 bytes | 
 public | comments             | table    | userCY2 | 8192 bytes | 
 public | comments_id_seq      | sequence | userCY2 | 8192 bytes | 
 public | schema_migrations    | table    | userCY2 | 16 kB      | 
(6 rows)
...
----+-----------------+-----------------------------------------+---------------
-------------+----------------------------
  1 | Annette's test1 | My test article for multisite failover. | 2021-03-13 15:
29:34.747517 | 2021-03-13 15:38:58.276829
(1 row)
----

=== Installing OADP for Backup and Restore

OADP (OpenShift APIs for Data Protection) is a community operator and is available in *OperatorHub*. If you are not finding OADP most likely the `community-operator` catalogsource is not deployed in your cluster.

We will be using OADP for the `Backup` and `Restore` APIs for the OpenShift metadata at a namespace level. This metadata is needed to restore the application on the *secondary cluster*.
...
NOTE: If using and OCS `S3` object bucket instead of on AWS reference these instructions https://github.com/konveyor/oadp-operator/blob/master/docs/noobaa/install_oadp_noobaa.md.
