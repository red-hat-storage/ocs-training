...
This is a general overview of the steps required to configure asynchronous DR capabilities via OCS *4.7* across two distinct OCP clusters separated by distance. 

NOTE: These steps are considered Dev Preview in OCS 4.7, and are provided for POCs purposes. They will be supported for production usage in a later OCS release.
...
. *Install OCS 4.7.* +
Install OCS 4.7 on primary and secondary OCP clusters and validate deployment.
...
Configure peer relationship between OCS clusters and create RBD Mirroring daemons.
...
In order to configure storage replication between the two OCP instances `OpenShift Container Storage` (OCS) must be installed first. Documentation for the deployment can be found at https://access.redhat.com/documentation/en-us/red_hat_openshift_container_storage.
...
You can validate the successful deployment of OCS on each OCP instance with the following command:
...
Before any new OCS volumes are created for replication a new *StorageClass* needs to be created using *CephBlockPool* `replicapool` that was created in prior section. 
...
NOTE: Your velero YAML file will be slightly different if using a `S3` object bucket from a different provider (GCP, Azure), from an external Ceph cluster with `RGW`, or from OCS `MCG`.
