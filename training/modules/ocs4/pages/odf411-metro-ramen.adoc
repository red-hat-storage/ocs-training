...
To validate that the *DRPolicy* is created successfully run this command on the *Hub cluster* for the first *DRPolicy* resource created. 

NOTE: Replace `<drpolicy_name>` with your unique name.

[source,role="execute"]
----
oc get drpolicy <drpolicy_name> -n openshift-dr-system -o jsonpath='{.status.conditions[].reason}{"\n"}'
----
.Example output.
----
Succeeded
...
Now each `DRCluster` needs to edited and your unique IP addresses added in this way after replacing `<drcluster_name>` with your unique name. 
...
IMPORTANT: The OpenShift cluster to `Fence` is the one where the application is currently running.
...
[source,role="execute"]
----
oc get drcluster.ramendr.openshift.io <drcluster_name> -n openshift-dr-system -o jsonpath='{.status.conditions[].reason}{"\n"}'
----
.Example output.
----
Fenced
...
The OpenShift cluster to be `Unfenced` is the one where the application is currently not running and the cluster that is `Fenced`.
